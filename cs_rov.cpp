...
#include "protocol_pult/protocol.h"
#include"protocol_pult/udp_protocol.h"
...
CS_ROV::CS_ROV(QObject *parent)
{
    ...
    AH127C = new AH127Cprotocol("ttyUSB1");  //ttyUSB1
    qDebug() << "Инициализировал БСО";


    digitalWrite (11, LOW);
    qDebug() << "digitalWrite (11, LOW)";
    ...
    QSettings settings_mp("settings_mp/settings_mp.ini", QSettings::IniFormat);
    settings_mp.beginGroup("PortMP");
    QString port_mp = "ttyUSB0";
    qint32 baudrate_mp = settings_mp.value("baudrate_mp", 115200).toInt();
    settings_mp.endGroup();

    manipulatorProtocol = new Manipulator_controller(port_mp, baudrate_mp);
    manipulatorProtocol->moveToThread(&manipulatorThread);
    QObject::connect(&manipulatorThread, &QThread::started, manipulatorProtocol, &Manipulator_controller::start);
    manipulatorThread.start();

    ps = new MS5837();
    ps->moveToThread(&psThread);
    QObject::connect(&psThread, &QThread::started, ps, &MS5837::startPS);

    psThread.start(); //ИЗМЕНЕНИЯ
    if (!psThread.isRunning()) {
        qDebug() << "Ошибка запуска потока psThread";
    }

    // Соединяем результат с обработчиком
    // Соединяем сигнал запроса глубины с слотом MS5837
    connect(this, &CS_ROV::requestDepth, ps, &MS5837::getDepth, Qt::QueuedConnection);

    // Соединяем результат с обработчиком
    connect(ps, &MS5837::depthReceived, this, &CS_ROV::processDepth);
...
//        QObject::connect(prUWB, &UWB::ProtocolUWB::renew, calib, &Calibration::newCalibration, Qt::BlockingQueuedConnection);
    ...
    manipulatorProtocol->setValues('3');
}
...
void CS_ROV::tick()
{
    qDebug() << "tick";
    // В MS5837::getDepth():
    //qDebug() << "Вызов getDepth() в потоке:" << QThread::currentThreadId();

    // В CS_ROV::processDepth():
    //qDebug() << "Обработка глубины в потоке:" << QThread::currentThreadId();
    readDataFromPult();
    readDataFromSensors();
    calibration();
    regulators();
    BFS_DRK(X[101][0], X[102][0], X[103][0] , X[104][0], X[105][0], X[106][0]);
    writeDataToManipulator();
    writeDataToVMA();
    writeDataToPult();
...
void CS_ROV::readDataFromPult()
{
    ...
    X[1005][0] = auvProtocol->rec_data.controlData.powoff;
    X[1006][0] = auvProtocol->rec_data.controlData.rotmanrt;
    X[1007][0] = auvProtocol->rec_data.controlData.rotmanlf;
    X[1008][0] = auvProtocol->rec_data.controlData.opening;
    X[1009][0] = auvProtocol->rec_data.controlData.gripping;


//    qDebug() << "X[1001][0]" << X[1001][0];
//    qDebug() << "X[1002][0]" << X[1002][0];
//    qDebug() << "X[1003][0]" << X[1003][0];
//    qDebug() << "X[1004][0]" << X[1004][0];

//    qDebug() << "X[1006][0]" << X[1006][0];
//    qDebug() << "X[1007][0]" << X[1007][0];
//    qDebug() << "X[1008][0]" << X[1008][0];
//    qDebug() << "X[1009][0]" << X[1009][0];
...
void CS_ROV::readDataFromSensors()
{
    ...
    } /*else qDebug() << "поток не запущен" << ;*/
    //qDebug() << "X[609][0] "<<X[609][0] ;
    emit requestDepth(); // Асинхронный запрос
        X[609][0] = ps->depth()+K[899];
        //qDebug() <<"ps->temperature: " << ps->temperature();
        //qDebug() <<"ps->pressure: " << ps->pressure();
        //qDebug() <<"ps->depth: " << ps->depth();
        //qDebug() <<"ps->altitude: " << ps->altitude();
...
//      integrate(X[69][0],X[91][0],X[91][1],dt); //интегрируем показание Z_rate для нахождения текущего угла курса
    ...
    qDebug() << "alternative_yaw_calculation X[91]" << X[91][0];
...
void CS_ROV::writeDataToPult()
{
    ...
    qDebug() << "pressure" << X[645][0];
    float temp = X[609][0];
    qDebug() << "Глубина бассейна: " << temp;
...
void CS_ROV:: timer_power_power()
  {
    if (auvProtocol->rec_data.pMode == power_Mode::MODE_2 || X[1000][0] == 1){
...
void CS_ROV::writeDataToManipulator()
{
    //qDebug() << "Я в writeDataToManipulator()";

    if (X[1001][0] != X[1006][0] && X[1006][0] == 1) {
        //qDebug() << "X[1006][0]";
        manipulatorProtocol->setValues('1');
        X[1001][0] = X[1006][0];
    }
    else if (X[1001][0] != X[1006][0] && X[1006][0] == 0) {
        //qDebug() << "X[1009][0]";
        manipulatorProtocol->setValues('0');
        X[1001][0] = 0;
    }
    else
        qDebug() << "X[1009][0]";

    if (X[1002][0] != X[1007][0] && X[1007][0] == 1) {
        //qDebug() << "X[1007][0]";
        manipulatorProtocol->setValues('2');
        X[1002][0] = X[1007][0];
    }
    else if (X[1002][0] != X[1007][0] && X[1007][0] == 0) {
        //qDebug() << "X[1009][0]";
        manipulatorProtocol->setValues('0');
        X[1002][0] = 0;
    }
    ...
    if (X[1003][0] != X[1008][0] && X[1008][0] == 1) {
        //qDebug() << "X[1008][0]";
        manipulatorProtocol->setValues('5');
        X[1003][0] = X[1008][0];
    }
    else if (X[1003][0] != X[1008][0] && X[1008][0] == 0) {
        //qDebug() << "X[1009][0]";
        manipulatorProtocol->setValues('3');
        X[1003][0] = 0;
    }
    ...
    if (X[1004][0] != X[1009][0] && X[1009][0] == 1) {
        manipulatorProtocol->setValues('4');
        X[1004][0] = X[1009][0];
    }
    else if (X[1004][0] != X[1009][0] && X[1009][0] == 0) {
        manipulatorProtocol->setValues('3');
        X[1004][0] = 0;
    }
    else
        qDebug() << "X[1009][0]";
    //manipulatorProtocol->setValues(manipulatorData);
}
