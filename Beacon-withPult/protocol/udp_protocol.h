...
public:
    ...
    Json_parser js;
    explicit UdpProtocol (QObject *parent = 0){
        ...
        m_ip_sender = js.set.sender_ip; //ip ПУ
        m_ip_receiver = js.set.receiver_ip; //ip ROV_Model
        m_port_sender = js.set.sender_port; //порт ПУ
        m_port_receiver = js.set.receiver_port; //порт ROV_Model
        m_frequency = js.set.sender_frequency; //частота передачи данных
